...
def takePic(objName, figNum):
	...
	#picPlane = [0, 5.235987756, 1.570796327, 0]
	#yRot = [0, -0.34906585, 0.523598776, 0]
	#zRot = [[0.523598776, 1.047197551, 1.570796327], [-0.523598776, 0, 0.523598776], [0.523598776, 1.047197551, 1.570796327], [0.523598776, 1.047197551, 1.570796327]]
	
	#finRot = [[0, 0, 110], [0, 20, 30], [90, 15, 60,],[0, 0,110], [180, 20, 35], [435, 15, 30], [0, 0, 120], [0, 20, 40], [90,0, -60],[0,0, -60], [0,0, -60] ]
	finRot = [[0, 20, 10]]

		#rotName = ["Front", "Top-Front", "Side-1", "Back", "Top-Back", "Side-2"]

	
	picRot = [radians(finRot[figNum][0]),radians(finRot[figNum][1]), radians(finRot[figNum][2])]
	bpy.context.object.rotation_euler = picRot
		
		
		#Set file path for the render
	bpy.context.scene.render.filepath = mkdir + imgdir + objName  + ".png"
...
#figNum =[23, 31, 32,  106, 108, 116, 266, 36, 101, 107]
figNum =[36]
...
for ii in range(0, len(figNum)):
	...
	takePic('Figure_' + str(figNum[ii]) + "_" +"Skel", ii)
	...
	takePic('Figure_' + str(figNum[ii]) + "_" + "Bulge", ii)
	#Take Skel video
