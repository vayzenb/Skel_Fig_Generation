...
def recordVid(objName,taper):
	...
	picPlane = [np.deg2rad(0)]
	
	for pp in range(0,len(picPlane)):

		#startRot = [[picPlane[pp], 0, -0.523598776], [picPlane[pp], 1.047197551, 1.047197551], [picPlane[pp], 0, 1.047197551]]
		#endRot = [[picPlane[pp], 0, 0.523598776], [picPlane[pp], 1.047197551, 2.094395102], [picPlane[pp], 0, 2.094395102]]
		orig = [picPlane[0],0,0]
		horzRot = [picPlane[0], 0, np.deg2rad(360)]
		vertRot = [picPlane[0], np.deg2rad(360), np.deg2rad(360)]
		...
		for kk in range(0, len(rotName)):
			...
			bpy.context.object.rotation_euler = orig
...
mkdir = "C:\\Users\\Vlad\\Documents\\Copy me\\Figure Generation\\"
imgdir = "Images\\"
viddir = "Videos\\"
...
for ii in range(0, len(figNum)):
	...
	recordVid("Figure_" + str(figNum[ii]),  "_" + taperObj[1])
			#bpy.context.area.type = oldContext
